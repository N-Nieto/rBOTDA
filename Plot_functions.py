...
def plot_violin_pair_comparison(df, compared_methods, subjects, save_dir="",
    ...
    ax = sns.violinplot(data=df_aux, x="Subject",
                        y=pd.to_numeric(df["Test Accuracy [%]"]),
                        ax=ax, hue="Method",
    ...
    add_stat_annotation(ax, data=df_aux, x="Subject",
                        y=pd.to_numeric(df["Test Accuracy [%]"]), hue="Method",
                        box_pairs=box_list, test='t-test_paired',
                        text_format='star', loc='inside',
                        pvalue_thresholds=[[1, "ns"], [0.01, "*"]],
                        verbose=False)
